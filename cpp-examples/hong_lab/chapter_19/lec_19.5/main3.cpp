 ...
 */
    // thread 와 async의 차이 : asyinc 는 join을 쓰지 않아도, 쓸 필요가 없다.
    // async를 받아주는 return type future를 쓰지 않면 동기처리 해버림! (의도치 않은 상황)
    /*
    std::async를 사용할 때 반환 값을 무시하면 경고가 발생합니다. std::async는 std::future 객체를 반환하는데, 
    이 반환 값을 처리하지 않으면 경고가 발생하고, 비동기 작업이 제대로 수행되지 않을 수 있습니다.
    */
    {
        std::async([]
        {
