...
main7.cpp 코드는 main2.cpp 코드와 같이  **4**개의 thread서 push_back하고, **4**개의 thread에서  dotProductLocalSum 계산하여 push_back 속도를 높이고자함
- 이때 lock_guard가 적용되는 범위를 최소화 하여 local_sum을 사용하는 방식을 사용하여 mutex의 오버헤드를 거의 없앰
- main2.cpp는 for loop 전체에 lock_guard가 있었던 반면, main7.cpp는 일부에서만 사용
 */
#include <iostream>
#include <chrono>
#include <random>
...
void dotProductLocalSum(const std::vector<int>& v0,
                        ...
                        unsigned long long& sum)
{
    ...
    // 스레드가 계산한 지역 합계를 메인 스레드의 합계에 추가
    {
...
int main()
{
    ...
    std::cout<<"dotProductLocalSum"<<std::endl;
    {
        const auto sta = std::chrono::steady_clock::now();
        ...
        for(unsigned t=0; t<n_threads;++t)
        {
            ...
            threads[t]=std::thread(dotProductLocalSum,std::ref(v0),std::ref(v1),t*n_per_thread,(t+1)*n_per_thread,std::ref(sum));
        }
