...
int main()
{
    ...
    std::cout<<"Naive"<<std::endl;
    {
        ...
        std::vector<std::thread> threads;
        threads.resize(dot_threads);

        const unsigned n_per_thread = n_data / dot_threads; // 100'000'000 / 4 ,assumes remainder = 0
        for(unsigned t=0; t<dot_threads;++t)
        {
