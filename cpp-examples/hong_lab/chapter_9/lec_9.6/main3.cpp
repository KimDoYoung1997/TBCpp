...
public:
    ...
    {
        assert(index >0); // assert를 선언함으로서, index가 0 미만인 값이 커스텀 배열로 들어오면 컴파일 에러를 의도적으로 발생시키자.
        int list_size = (sizeof(m_list)/(sizeof(int)));
        // cout << list_size<<endl;
        assert(index<list_size); // assert를 선언함으로서, index가 m_list 의 원소개수 미만이도록 설정
        return m_list[index];
    }
